...
# saved as "/home/pi/sensor_menu-2017-11-07-19-24-19.py"
...
def execute(hat, selection):
    if selection == 'T':
        hat.show_message('Temperature: %.1fC' % hat.temp, 0.25, Rd)
    elif selection == 'P':
        hat.show_message('Pressure: %.1fmbar' % hat.pressure, 0.25, Gn)
    elif selection == 'H':
        hat.show_message('Humidity: %.1f%%' % hat.humidity, 0.25, Bl)
    else:
        return True
    return False
